...
def soft_nms(dets, iou_thr=0.7, method='gaussian', sigma=0.15):
    """
...
def get_prediction(vid_name, data_dict, dataset, args):
    ...
    """
    prop_v_cas = data_dict.get('prop_v_cpm_cas', data_dict.get('prop_v_pcm_cas', data_dict.get('prop_v_refined_cas', data_dict['prop_v_cas'])))[0]
    prop_f_cas = data_dict.get('prop_f_cpm_cas', data_dict.get('prop_f_pcm_cas', data_dict.get('prop_f_refined_cas', data_dict['prop_f_cas'])))[0]
    prop_v_attn = data_dict['prop_v_attn'][0]
    prop_f_attn = data_dict['prop_f_attn'][0]
    prop_v_iou = data_dict.get('prop_v_refined_iou', data_dict['prop_v_iou'])[0]
    prop_f_iou = data_dict.get('prop_f_refined_iou', data_dict['prop_f_iou'])[0]
    prop_v_dom_logit = data_dict.get('prop_v_dom_logit')
    prop_f_dom_logit = data_dict.get('prop_f_dom_logit')
    prop_cpm_bonus = data_dict.get('prop_cpm_bonus', data_dict.get('prop_pcm_bonus'))
    idx_all = list(dataset.videonames).index(vid_name)
    proposals = data_dict.get('prop_segments')
    ...
    if prop_v_dom_logit is None or prop_f_dom_logit is None:
        prop_dom = torch.zeros_like(prop_cas)
    else:
        prop_dom = torch.sigmoid(0.5 * (prop_v_dom_logit[0] + prop_f_dom_logit[0]))
    pred_vid_score = (prop_cas * prop_attn).sum(0) / (prop_attn.sum(0) + 1e-6)
    pred_vid_score = pred_vid_score[:-1]

    prop_score = prop_cas * prop_attn * prop_iou * (1 + args.dominance_alpha * prop_dom)
    if prop_cpm_bonus is not None:
        prop_cpm_bonus = prop_cpm_bonus[0]
        if prop_cpm_bonus.shape[1] > 1:
            identity = torch.eye(prop_cpm_bonus.shape[1], device=prop_cpm_bonus.device, dtype=torch.bool).unsqueeze(0)
            other_max = prop_cpm_bonus.unsqueeze(2).expand(-1, -1, prop_cpm_bonus.shape[1]).masked_fill(identity, -1e4).max(dim=2)[0]
            prop_cpm_bonus = F.relu(prop_cpm_bonus - other_max)
        topk_vid = max(1, prop_cpm_bonus.shape[0] // max(1, args.k))
        support_vid_score = torch.topk(prop_cpm_bonus, k=topk_vid, dim=0).values.mean(dim=0)
        pred_vid_score = pred_vid_score * (1 + args.cpm_score_alpha * support_vid_score)
        prop_cpm_factor = torch.cat((
            1 + args.cpm_score_alpha * prop_cpm_bonus,
            torch.ones_like(prop_score[:, -1:])
        ), dim=1)
        prop_score = prop_score * prop_cpm_factor
