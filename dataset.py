...
def resolve_dir(path, fallback_parts):
    candidate = Path(path)
...
class VideoDataset:
    def __init__(self, args):
        ...
        self.dataset_root = resolve_dir(
            args.dataset_root,
            fallback_parts=(
                ('main', 'data', self.dataset_name),
                ('base', 'data', self.dataset_name),
            ),
        )
        self.proposal_dir = resolve_dir(
            args.proposal_dir,
            fallback_parts=(
                ('main', 'proposals'),
                ('base', 'proposals'),
        ...
        self.path_to_features = os.path.join(self.dataset_root, self.dataset_name + "-I3D-JOINTFeatures.npy")
        self.path_to_annotations = os.path.join(self.dataset_root, self.dataset_name + "-Annotations/")

        if not os.path.exists(self.path_to_features):
            raise FileNotFoundError('Feature file not found: {}'.format(self.path_to_features))
        if not os.path.isdir(self.path_to_annotations):
            raise FileNotFoundError('Annotation directory not found: {}'.format(self.path_to_annotations))
    ...
    def get_proposals(self, only_test=True):
        ...
        """
        proposals_file_test = os.path.join(self.proposal_dir, 'detection_result_{}_test.json'.format(self.base_method))
        with open(proposals_file_test, "r") as f:
            proposals_json_test = json.load(f)["results"]
        if not only_test:
            proposals_file_train = os.path.join(self.proposal_dir, 'detection_result_{}_train.json'.format(self.base_method))
